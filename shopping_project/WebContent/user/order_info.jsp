...
<body>
	...
	<div class="container">
			...
			<div style="margin: 0 auto; margin-top: 10px; width: 950px;">
				...
				<table class="table table-bordered">
					<tbody>
						...
						<c:forEach var="item" items="${od.listItem }">

							<tr class="active">
								<td width="60" width="40%"><input type="hidden" name="id"
									value="22"> <img src="${item.p.pimage}" width="70"
									height="60"></td>
								<td width="30%"><a target="_blank">${item.p.pname }</a></td>
								<td width="20%">￥${item.p.shop_price }</td>
								<td width="10%">${item.count }</td>
								<td width="15%"><span class="subtotal">￥${item.count * item.p.shop_price }</span>
								</td>
							</tr>
							<c:set var="sum" value="${sum + item.count * item.p.shop_price }"></c:set>
						</c:forEach>
					</tbody>
				</table>
			</div>
		...
		<div>
			<hr />
			<form  action="${pageContext.request.contextPath}/payServlet" method="post" id ="orderForm" class="form-horizontal"
				style="margin-top: 5px; margin-left: 150px;">
				<input type="hidden" name ="method" value = "payForGoods" />
